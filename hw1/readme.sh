...
#this is a test to make sure that git commit works
...
for i in {1..9};do # The for loop is a command designed to run commands many times based on the variables set by the user. In this example I am interested in renaming files containing 0<n<10. This for loop can differentiate between a 1 by itself and a 1 with a 0 or another number eg 10, 11...etc. After identifying the files with this variable, the "do" command is followed by "cp". For syntax requires "do" and a command. Cp will copy my files with the number variable and rename them according to the script with a leading 0. However, the cp command copies my files and keeps my originals so I must also include a remove command eg rm and specify the old files to remove.
